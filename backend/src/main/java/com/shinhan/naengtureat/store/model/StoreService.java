...
public class StoreService {
	...
	public List<StorePriceDTO> getStorePriceList(List<Long> ingredientIds) {
		List<StorePriceDTO> storePrices = storeProductRepository.findStorePricesByIngredients(ingredientIds);

		if (storePrices.isEmpty()) {
			throw new NoSuchElementException("해당 재료를 판매하는 스토어가 없습니다.");
		}
