...
public class MealPlanController {
	...
	public ResponseEntity<Object> getDailyMealPlanList(@PathVariable("day") String day) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(BaseResponse.builder().message("식단 일간 조회 중 오류 발생했습니다: " + e.getMessage()).build());
		}
	}
	...
	public ResponseEntity<Object> updateBudget(@RequestBody Map<String, Integer> requestBody) {
		...
		if (budget == null || budget < 0) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(BaseResponse.builder().message("유효한 예산 값을 입력하세요.").build());
		}
	...
	public ResponseEntity<Object> getWeeklyMealPlanList(@PathVariable("day") String day) {
		try {
			// 세션에서 로그인된 사용자 정보 가져오기
			Long memberId = 2L; // security 적용시 코드 수정 필요(WebBoardController SecurityContextHolder, MemberService
								// 참고)
	...
	public ResponseEntity<Object> updateMealPlan(@PathVariable("mealPlanId") Long mealPlanId,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(BaseResponse.builder().message("저장된 식단 이동 중 오류 발생했습니다: " + e.getMessage()).build());
		}
	}
}