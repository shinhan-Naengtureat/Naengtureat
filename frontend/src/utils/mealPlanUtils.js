...
export const calculateMealStats = (mealData, calendarValue, AVERAGE_MEAL_COST) => {
  ...
  const mealsInMonth = mealData.filter((meal) => meal.date.startsWith(currentMonth));
  return {
    totalMeals: mealsInMonth.length, // 전체 식단 개수
    completedMeals: mealsInMonth.filter((meal) => meal.check).length, // 이행한 식단 개수
    estimatedExpense: mealsInMonth.length * AVERAGE_MEAL_COST, // 예상 외식 비용
    usedAmount: Math.floor(mealsInMonth.reduce((sum, meal) => // 사용한 금액(레시피 가격/레시피 인분)
