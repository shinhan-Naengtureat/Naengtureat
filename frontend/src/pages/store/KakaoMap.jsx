    ...
    return (
        <>
            ...
            <Map center={center} style={{width: "100%", height: "350px"}} level={5}>
                <MapMarker position={center} image={{src: `${STORE_IMAGE_PATH}/marker-home.png`, size: {width: 35, height: 35}}} title='우리집'></MapMarker>

                {/* '대형슈퍼' 검색 결과 마커 표시 */}
                {places.map((place, index) => (
                <MapMarker key={index} position={{lat: Number(place.y), lng: Number(place.x)}} image={{src: `${STORE_IMAGE_PATH}/marker-market.png`, size: {width: 35, height: 35}}} title={place.place_name} />
                ))}
            </Map>}
        </>
    );
